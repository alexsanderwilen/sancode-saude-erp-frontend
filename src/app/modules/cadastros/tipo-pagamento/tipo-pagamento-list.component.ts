...
export class TipoPagamentoListComponent {
  ...
  constructor(private service: TipoPagamentoService, private dialog: MatDialog, private fb: FormBuilder, private agGridLocaleService: AgGridLocaleService) {
    this.gridOptions = { ...this.agGridLocaleService.getDefaultGridOptions(), pagination: true, paginationPageSize: 20 };
    this.load();
  }

  load(): void { this.service.getTiposPagamento().subscribe(d => this.rowData = d); }
  openDialog(row?: any): void {
    const form: FormGroup = this.fb.group({ id: [row?.id || null], descricao: [row?.descricao || '', Validators.required] });
    const ref = this.dialog.open(TipoPagamentoDialogComponent, { width: '520px', data: { form, title: row ? 'Editar' : 'Novo' } });
...
export class TipoPagamentoDialogComponent {
  constructor(@Inject(MAT_DIALOG_DATA) public data: { form: FormGroup, title: string }, private ref: MatDialogRef<TipoPagamentoDialogComponent>) {}
