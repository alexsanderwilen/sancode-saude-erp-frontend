...
export class PlanoStatusListComponent {
  ...
  constructor(private service: PlanoStatusService, private dialog: MatDialog, private fb: FormBuilder, private agGridLocaleService: AgGridLocaleService) {
    this.gridOptions = { ...this.agGridLocaleService.getDefaultGridOptions(), pagination: true, paginationPageSize: 20 };
    this.load();
  }

  load(): void { this.service.getPlanosStatus().subscribe(d => this.rowData = d); }

  openDialog(row?: any): void {
    const form: FormGroup = this.fb.group({ id: [row?.id || null], descricao: [row?.descricao || '', Validators.required] });
    const ref = this.dialog.open(PlanoStatusDialogComponent, { width: '520px', data: { form, title: row ? 'Editar' : 'Novo' } });
...
@Component({
  selector: 'app-plano-status-dialog',
  standalone: true,
...
export class PlanoStatusDialogComponent {
  constructor(@Inject(MAT_DIALOG_DATA) public data: { form: FormGroup, title: string }, private ref: MatDialogRef<PlanoStatusDialogComponent>) {}
