...
export class TipoContratacaoListComponent {
  ...
  constructor(private service: TipoContratacaoService, private dialog: MatDialog, private fb: FormBuilder, private agGridLocaleService: AgGridLocaleService) {
    this.gridOptions = { ...this.agGridLocaleService.getDefaultGridOptions(), pagination: true, paginationPageSize: 20 };
    this.load();
  }

  load(): void { this.service.getTiposContratacao().subscribe(d => this.rowData = d); }

  openDialog(row?: any): void {
    const form: FormGroup = this.fb.group({ id: [row?.id || null], descricao: [row?.descricao || '', Validators.required] });
    const ref = this.dialog.open(TipoContratacaoDialogComponent, { width: '520px', data: { form, title: row ? 'Editar' : 'Novo' } });
...
@Component({
  selector: 'app-tipo-contratacao-dialog',
  standalone: true,
...
export class TipoContratacaoDialogComponent {
  constructor(@Inject(MAT_DIALOG_DATA) public data: { form: FormGroup, title: string }, private ref: MatDialogRef<TipoContratacaoDialogComponent>) {}
